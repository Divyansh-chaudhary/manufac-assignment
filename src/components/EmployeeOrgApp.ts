...
export default class EmployeeOrgApp implements IEmployeeOrgApp {
  ...
  move(employeeID: number, supervisorID: number): void {
    ...
    if (!fromSupervisor || !toSupervisor || fromSupervisor === toSupervisor)
      return;
  ...
  private findSupervisorOfEmployee(
    root: Employee | undefined,
    id: number
  ): Employee | undefined {
    if (root === undefined) return undefined;
  ...
  private removeEmployeeFromSubordinates(
  ...
  private addEmployeeToSubordinates(
    supervisor: Employee | undefined,
